...
export function filter<A, B extends A>(predicate: (a: A) => a is B): (self: Observable<A>) => Observable<B>;
export function filter<A>(predicate: (a: A) => boolean): (self: Observable<A>) => Observable<A>;
export function filter<A>(predicate: (a: A) => boolean): (self: Observable<A>) => Observable<A> {
...
export const bufferEvery = (n: number) => <A>(self: Observable<A>) => {
    return from<A[]>((observer) => {
        let buffer: A[] = [];
        return self["|>"](subscribe({
            ...observer,
...
export const takeFirst = (n: number) => <A>(self: Observable<A>) => {
    return from<A>((observer) => {
        let count = n;
        return self['|>'](subscribe({
            ...observer,
...
export const subscribe = <A>(obs: Subscriber<A>) => (self: Observable<A>): Unsubscribe => {
